...
| `ml-traffic-rl-navigation` | Theme rewrite | Machine-learning group report titled "Intelligent Traffic Congestion Prediction and Navigation System using LSTM and Reinforcement Learning"; PDF appendix/code discussion for the RL module; public reference repository `YuweiZhang-002/Navigation-System-based-on-LSTM-and-Q-Learning` linked in the report. | Supports Q-learning route planning on a 3x4 traffic grid, Bellman update, congestion/distance reward design, learning-rate sensitivity framing, and LSTM-to-RL forecast interface. The reference repository has no explicit license, so this project is an original clean public remake rather than copied code. |
